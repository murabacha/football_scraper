import scrapy
from football_scraper.items import TransferItem
import json
import datetime

class LeagueSpiderSpider(scrapy.Spider):
    name = "league_transfers_spider"
    custom_settings = {
        'ITEM_PIPELINES': {
            'football_scraper.pipelines.SaveLeagueTransfersPipeline': 800,
    ...
    def parse(self, response):
        for url in self.leages_urls:
            transfers_url = 'https://onefootball.com' + url + '/transfers'
            yield scrapy.Request(url=transfers_url, callback=self.parse_league)
    
    def parse_league(self, response):
        transfer_item = TransferItem()
        league_name = response.css('p.title-1-bold-druk.EntityTitle_title__sWSgU::text').get()
        transfer_item['league'] = league_name
        transfer_item['transfers'] = []
        transfer_containers = response.css('li.TransferCardsList_transferCard__ZGCh4')
        for container in transfer_containers:
            transfer_type = container.css('header.title-8-bold.TransferCard_title__uHrSa::text').get()
            if transfer_type == 'DONE DEAL':
                transfer_type = 'DONE DEAL'
                date = container.css('header.title-8-bold.TransferCard_title__uHrSa time::text').get()
                if date == 'Today':
                    ...
                    date = date
                player_img =  container.css('img.TransferCard_contentPlayerImage__OXso4::attr(src)').get()
                player_name = container.css('p.title-7-bold.TransferCard_contentPlayerInfoName__pHlPG::text').get()
                position = container.css('p.title-8-regular.TransferCard_contentPlayerInfoPosition__5VIBY::text').get()
                team_logo = container.css('img.EntityLogo_entityLogoImage__4X0wF::attr(src)').get()
                old_team = container.css('p.title-8-regular.TransferCard_secondaryContentText__FozCP::text').get()
                new_team  = container.css('p.title-8-regular.TransferCard_secondaryContentText__FozCP.TransferCard_secondaryContentTextRight__dZyex::text').get()
                fee = container.css('p.title-8-bold.TransferCard_footer__xus_5::text').get() or 'Undisclosed'
                fee = fee.replace('?', '')
                data = {
                    'league': league_name,
                ...
                transfer_item['transfers'].append(data)
            else:
                transfer_type = container.css('header.title-8-bold.TransferCard_title__uHrSa::text').get()
                date = container.css('header.title-8-bold.TransferCard_title__uHrSa time::text').get()
                if date == 'Today':
                ...
                position = container.css('p.title-8-regular.TransferCard_contentPlayerInfoPosition__5VIBY::text').get()
                team_logo = container.css('img.EntityLogo_entityLogoImage__4X0wF::attr(src)').get()
                until = container.css('p.title-8-regular.TransferCard_contentRenewal__dLjKO::text').get()
                fee = container.css('p.title-8-bold.TransferCard_footer__xus_5::text').get() or 'RENEWAL'
                data = {
                    'league': league_name,
                    'transfer_type': transfer_type,
                    'date': date,
